            ...
            <div class="card shadow p-3 bg-body rounded">
                <div class="card-body">
                    ...
                    <ul class="list-group list-group-flush">
                        @foreach ($asignatura->recent_course as $curso)
                            <li class="list-group-item">
                                <a href="{{ route('cursos.show', $curso->id) }}"
                                    class="link-body-emphasis link-offset-2 link-underline-opacity-0 link-underline-opacity-75-hover">{{ Str::limit($curso->title, 40) }}</a>
                                <small class="text-info float-end">{{ (new DateTime($curso->created_at))->format('d/m/Y H:i') }}</small>
                            </li>
                        @endforeach
                    </ul>
